...
class AwsRedis:
    ...
    def get_region_aws_redis_instances(self, result_queue, client, region, account):
        aws_redis = {region: {}}
    ...
    def get_account_aws_redis_instances(self, account):
        ...
        for region in self.regions:
            ...
            t = threading.Thread(target=self.get_region_aws_redis_instances, args=(result_queue, aws_client, region, account))
            threads.append(t)
            t.start()
        # 等待所有线程完成
        for t in threads:
